...
public class GameOverController {
	...
	public void give(TextToSpeechBackground textToSpeechBackground, Boolean textToSpeech, BufferedImage bufferedImage) {
	 ...
	 * https://stackoverflow.com/questions/30970005/bufferedimage-to-javafx-image on
	 * 14/10/22 posted by Dan
	 *
	 * @param bufferedImage takes a buffered image that is a screenshot of the final
	 *                      drawing the user did
	...
	public void setWinLoseLabel(boolean winLose, CanvasController ctrl, int overallDif)
		...
		spreadSheetReaderWriter.updateResult(winLose, currentUsername, overallDif); // writes over old file to update
		// win/loss record
	}
	...
	private void onSave() {
		...
		if (file != null) {
			try {
				ImageIO.write(canvasController.getCurrentSnapshot(), "bmp", file); // creates a new image to save to the
				// users location
	...
	private void onClickMenu() throws IOException, CsvException {
		...
		FXMLLoader loader = new FXMLLoader(App.class.getResource("/fxml/main_menu.fxml")); // reset to a new word_page
		...
		MainMenuController ctrl = loader.getController(); // gets controller of new page to pass text to speech
		ctrl.give(textToSpeechBackground, textToSpeech); // passes text to speech instance and boolean to next page
	...
	private void onPlayAgain() throws IOException, URISyntaxException, CsvException {
		...
		FXMLLoader loader = new FXMLLoader(App.class.getResource("/fxml/word_page.fxml")); // reset to a new word_page
		// where a new word will be
		// generated
		Scene scene = new Scene(loader.load(), 1000, 680);
		stage.setScene(scene);
		...
		WordPageController ctrl = loader.getController(); // gets controller of new page to pass text to speech
		ctrl.give(textToSpeechBackground, textToSpeech); // passes text to speech instance and boolean to next page
