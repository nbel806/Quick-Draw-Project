...
public class WordPageController {
	...
	private void onReady() throws IOException {
		...
		stage.show();
		CanvasController canvasController = loader.getController(); // gets the newly created controller for next page
		canvasController.setWordLabel(currentWord); // passes the current word so that the next screen can display it
