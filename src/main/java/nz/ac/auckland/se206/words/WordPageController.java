...
public class WordPageController {
	...
	@FXML
	private Button plusWords;
	@FXML
	private Button minusWords;
	@FXML
	 ...
	 */
	public void getUsername(String username, String profilePic) throws IOException, URISyntaxException, CsvException {
	...
	private void onHiddenWordMode() throws IOException, URISyntaxException, CsvException, WordNotFoundException {
		...
		if (isHiddenWordMode == true) {
			...
			// Set style
			modeButton.setStyle("-fx-border-color: black; -fx-background-color: #45f248; "
			...
			// Set style
			modeButton.setStyle("-fx-border-color: black; -fx-background-color: #EB4A5A; "
					+ "-fx-border-radius: 10px; -fx-background-radius: 10px;");
	 ...
	 */
	@FXML
	private void onReady() throws IOException, WordNotFoundException, URISyntaxException, CsvException {
		Stage stage = (Stage) readyButton.getScene().getWindow(); // uses the ready button to fine the stage
		...
		if (!isHiddenWordMode) {
			...
			CanvasController canvasController = loader.getController(); // gets the newly created controller for next
			// page
			canvasController.setTimeAccuracy(time, accuracy, confidence, words, overallDif);
			canvasController.setWordLabel(currentWord); // passes the current word so that the next screen can display
			// it
			canvasController.give(textToSpeechBackground, textToSpeech); // passes the background threaded text to
			// speech
			...
			Scene scene = new Scene(loader.load(), 1000, 680);
			stage.setScene(scene);
			HiddenWordCanvasController hiddenWordCanvasController = loader.getController(); // gets the newly created
			// controller for next page
			hiddenWordCanvasController.setTimeAccuracy(time, accuracy, confidence, words, overallDif);
			hiddenWordCanvasController.give(textToSpeechBackground, textToSpeech); // passes the background threaded
			// text to speech
			// and whether it is on or not
	...
	private void setFirstDifficulty(String currentUsername) throws IOException, CsvException {
		...
		confidence = sheetReaderWriter.getUsersConfidence(currentUsername); // gathers conf from prev settings
